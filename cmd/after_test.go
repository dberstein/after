...
func Test_escapeArgs(t *testing.T) {
	type args struct {
		args []string
	}
	tests := []struct {
		...
		{"spaces", args{[]string{" abc ", "a b c"}}, []string{"' abc '", "'a b c'"}},
		{"quotes", args{[]string{"\"a b c\"", "'a b c'"}}, []string{"'\"a b c\"'", "\"'a b c'\""}},
		{"special", args{[]string{"$val#ue"}}, []string{"'$val#ue'"}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
